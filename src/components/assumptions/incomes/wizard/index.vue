<template>
  <div class="addIncome bg-light">
    <div class="step-container">
      <div v-if="!isCompleted" class="stepper mx-auto bg-light text-regular">
        <form-wizard :title="'Create New Income'" :subtitle="''" :stepSize="'xs'" :color="'#525670'"  :errorColor="'#525670'" ref="step">
      ...
      <div v-else class="complete mx-auto bg-light text-regular">
        <span class="title">Create New Income</span>
        <div class="mark depth-1">
          <div class="circle-check-mark"></div>
          <p>Income Added</p>
        </div>
         <b-button class="go-incomes-link pull-right mt-5" to="/assumptions/incomes">Go to incomes</b-button>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    async onComplete () {
      if (this.validateStep('step3')) {
        try {
          await axios.post(`${process.env.ROOT_API}/incomes`, this.finalModel)
