  ...
  <div class="step1 d-flex">
    ...
    <b-container>
      ...
      <b-form-group id="subtypeHorizontal"
                ...
                :state="!$v.debt_subtype_id.$error">
        <b-form-select :options="subtypeOptions" :value-field="'subtype_id'" :text-field="'subtype'" v-model="debt_subtype_id" id="subtypeSelect" :state="!$v.debt_subtype_id.$error" @input="$v.debt_subtype_id.$touch()"/>
      </b-form-group>
...
export default {
  ...
  async mounted () {
    try {
      let response = await axios.get(`${process.env.ROOT_API}/debts/subtypes`)
  ...
  computed: {
    ...
    invalidSubtypeFeedback () {
      if (this.$v.debt_subtype_id.$error) {
