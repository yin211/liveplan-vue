    ...
    <b-container>
      ...
      <b-form-group id="amountHorizontal"
        ...
        <div class="d-flex">
          ...
          <b-form-input v-model="amount" min="0" max="100000" step="500" class="slider" type="range"></b-form-input>
        </div>
      </b-form-group>
...
export default {
  ...
  async mounted () {
    try {
      let response = await axios.get(`${process.env.ROOT_API}/incomes/subtypes`)
  ...
  validations: {
    ...
    form: ['name', 'description', 'income_subtype_id', 'amount', 'amount_recurrence', 'category_id', 'person_id']
  },
  methods: {
    validate () {
      this.$v.form.$touch()
      ...
      if (isValid) {
        let data = {
          name: this.name,
          income_subtype_id: this.income_subtype_id,
  ...
  computed: {
    ...
    invalidSubtypeFeedback () {
      if (this.$v.income_subtype_id.$error) {
